...
struct optpfor_block {
        ...
        uint32_t /* sum_of_values */, size_t n)
    {
        ...
        if (n == block_size) {
            ret = reinterpret_cast<uint8_t const*>(optpfor_codec.decodeBlock(reinterpret_cast<uint32_t const*>(in),
                out, out_len));
            assert(out_len == n);
        } else {
...
struct simple16_block {
    static const uint64_t block_size = 128;
    using codec_type = Simple16<false>;
    static codec_type simple16_codec;
