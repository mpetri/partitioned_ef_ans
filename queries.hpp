        ...
        while (true) {
            ...
            if (pivot_id == ordered_enums[0]->docs_enum.docid()) {
                ...
                for (scored_enum* en : ordered_enums) {
                    ...
                    score += en->q_weight * scorer_type::doc_term_weight(en->docs_enum.freq(), norm_len);
                    en->docs_enum.next();
                }
                ...
                for (size_t i = next_list + 1; i < ordered_enums.size(); ++i) {
                    if (ordered_enums[i]->docs_enum.docid() < ordered_enums[i - 1]->docs_enum.docid()) {
    ...
    uint64_t operator()(Index const& index, term_id_vec terms)
    {
        ...
        while (candidate < index.num_docs()) {
            ...
            if (i == enums.size()) {
                ...
                float score = 0;
                for (i = 0; i < enums.size(); ++i) {
                    score += enums[i].q_weight * scorer_type::doc_term_weight(enums[i].docs_enum.freq(), norm_len);
                }
        ...
        while (non_essential_lists < ordered_enums.size() && cur_doc < index.num_docs()) {
            ...
            for (size_t i = non_essential_lists; i < ordered_enums.size(); ++i) {
                if (ordered_enums[i]->docs_enum.docid() == cur_doc) {
                    score += ordered_enums[i]->q_weight * scorer_type::doc_term_weight(ordered_enums[i]->docs_enum.freq(), norm_len);
            ...
            if (m_topk.insert(score)) {
                ...
                while (non_essential_lists < ordered_enums.size() && !m_topk.would_enter(upper_bounds[non_essential_lists])) {
