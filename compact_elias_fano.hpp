...
struct compact_elias_fano {
    ...
    public:
        ...
        uint64_t prev_value() const
        {
            ...
            uint64_t prev_low = m_bv->get_word56(m_of.lower_bits_offset + prev_pos * m_of.lower_bits)
                & m_of.mask;
    ...
    private:
        value_type QS_NOINLINE slow_move(uint64_t position)
        {
            ...
            if (position > m_position
                ...
                m_high_enumerator = succinct::bit_vector::unary_enumerator(*m_bv, m_of.higher_bits_offset + high_pos);
